...
const TerritoryCard = ({ territory }: any) => {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group overflow-hidden">
      ...
      <div className="p-5">
        <h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{territory.name}</h3>
        ...
        <div className="mt-6 pt-4 border-t border-gray-50 grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Último trabalho</span>
          ...
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Publicador</span>
            <p className="text-xs font-semibold text-gray-700 flex items-center gap-1">
        ...
        <Link to={`/territory/${territory.id}`} className="mt-5 w-full flex items-center justify-center gap-2 bg-gray-50 hover:bg-blue-600 hover:text-white text-gray-600 py-2.5 rounded-xl text-sm font-bold transition-all">
          Ver Detalhes
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Territórios</h1>
          <p className="text-gray-500">Gerencie e acompanhe o status dos campos.</p>
        </div>
        <button
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none bg-white"
            value={searchTerm}
        ...
        <div className="flex gap-4">
          <div className="relative">
            ...
            <select
              className="pl-11 pr-4 py-3 rounded-2xl border border-gray-200 outline-none bg-white appearance-none min-w-[160px] font-medium text-gray-600"
              value={filterStatus}
          ...
          <div className="flex bg-white p-1 rounded-2xl border border-gray-200 shadow-sm">
            <button
              ...
              className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-blue-50 text-blue-600' : 'text-gray-400'}`}
            >
              <Grid size={20} />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-xl transition-all ${viewMode === 'list' ? 'bg-blue-50 text-blue-600' : 'text-gray-400'}`}
            >
              <ListIcon size={20} />
            </button>
          </div>
      ...
      {filteredTerritories.length > 0 ? (
        ...
        <div className="bg-white py-20 rounded-3xl border-2 border-dashed border-gray-200 flex flex-col items-center text-center">
          <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center text-gray-300 mb-4">
            <Search size={40} />
          </div>
          <h3 className="text-xl font-bold text-gray-900">Nenhum território encontrado</h3>
